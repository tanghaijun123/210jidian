...
//void menu(void){
//    static int parameterIndex = 0; // 用于跟踪当前参数索引
//    const char* parameters[] = {"温度", "压力", "位移", "流量"}; // 可选择的参数列表
//    const int numParameters = sizeof(parameters) / sizeof(parameters[0]); // 参数的总数
//
//    OLED_NewFrame();
//    OLED_PrintString(40, 0, "信号类型", &font12x12, OLED_COLOR_NORMAL);
//
//    // 显示参数列表，并在当前选中的参数前添加 * 号
//    for (int i = 0; i < numParameters; ++i)
//    {
//        if (i == parameterIndex)
//            OLED_PrintString(10, 16 + i * 12, "* ", &font12x12, OLED_COLOR_NORMAL);
//        else
//            OLED_PrintString(10, 16 + i * 12, "  ", &font12x12, OLED_COLOR_NORMAL);
//        OLED_PrintString(22, 16 + i * 12, parameters[i], &font12x12, OLED_COLOR_NORMAL);
//    }
//
//    OLED_ShowFrame();
...
//        // 当按键被按下时执行的操作
//        // 切换到下一个参数
//        parameterIndex = (parameterIndex + 1) % numParameters;
//        HAL_Delay(100); // 延时一段时间以避免连续触发
//        while (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_2) == GPIO_PIN_RESET); // 等待按键释放
//    }
//}
//void displayMenu1(int posY) {
//    OLED_NewFrame();
